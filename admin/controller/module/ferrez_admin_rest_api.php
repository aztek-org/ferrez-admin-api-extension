...
class FerrezAdminRestApi extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		// Stored values may have been HTML-escaped by the Request class. Decode for display/processing.
		$raw_permissions = (string)$this->config->get('module_ferrez_admin_rest_api_permissions');
		$data['module_ferrez_admin_rest_api_permissions'] = htmlspecialchars_decode($raw_permissions, ENT_QUOTES);
		...
		if ($data['module_ferrez_admin_rest_api_permissions'] !== '') {
			// Decode any HTML escaping that may have been applied before attempting to parse JSON.
			$decoded_permissions = json_decode(htmlspecialchars_decode($data['module_ferrez_admin_rest_api_permissions'], ENT_QUOTES), true);
	...
	public function save(): void {
		...
		$permissions_json_raw = (string)($this->request->post['module_ferrez_admin_rest_api_permissions'] ?? '');
		// Request::clean applies htmlspecialchars() which will encode quotes to &quot;.
		// Decode any HTML escaping before validating JSON.
		$permissions_json = htmlspecialchars_decode($permissions_json_raw, ENT_QUOTES);
